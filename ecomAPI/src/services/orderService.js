    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.addressUserId || !data.typeShipId) {
                ...
                const affiliateAttribution = data.affiliateAttribution || getAffiliateAttribution(data.req);
                ...
                if (affiliateAttribution && affiliateAttribution.kolId && affiliateAttribution.affiliateId) {
                    ...
                    if (attributionResult.errCode === 0) {
                        ...
                        console.error('Error processing multi-product attribution:', attributionResult.errMessage);
                    }
                } else {
                    ...
                    async function (error, payment) {
                        if (error) {
                            ...
                            if (affiliateAttribution && affiliateAttribution.kolId && affiliateAttribution.affiliateId) {
                                ...
                                if (attributionResult.errCode === 0) {
                                    console.log('Multi-product attribution processed successfully for PayPal order:', {
                                        orderId: product.dataValues.id,
                                        totalItems: attributionResult.data.totalItems,
                                        attributedItems: attributionResult.data.attributedItems,
                                        totalCommissions: attributionResult.data.totalCommissions,
                                    ...
                                    console.error('Error processing multi-product attribution for PayPal order:', attributionResult.errMessage);
                                }
                            } else {
                                console.log('No valid affiliate attribution data found for PayPal order');
                            }
                            let res = await db.ShopCart.findOne({
                                where: { userId: data.userId, statusId: 0 },
                            });
